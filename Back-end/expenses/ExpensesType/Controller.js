...
export const GetExpenseType = catchAsync(async (req, res, next) => {
  ...
  if(!expenseType){
    return next(new APPError(`Expense type with ID ${req.params.id} not found`, 404));
  }
  res.status(200).json({ success: true, expenseType });
});

export const UpdateExpenseType = catchAsync(async (req, res, next) => {
  const expenseType = await ExpenseType.findByIdAndUpdate(req.params.id, req.body, { new: true });
  if(!expenseType){
    return next(new APPError(`Expense type with ID ${req.params.id} not found`, 404));
  }
  res.status(200).json({ success: true, expenseType });
