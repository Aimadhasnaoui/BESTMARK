...
import { UpdatePurchase } from "@/Servises/Purchases";
...
export default function Update({ isUpdating, setIsUpdating, selectedPurchase }) {
  ...
  useEffect(() => {
    if (selectedPurchase) {
      reset({
        ...
        purchaseDate: selectedPurchase.purchaseDate ? new Date(selectedPurchase.purchaseDate).toISOString().split('T')[0] : "",
        items: selectedPurchase.items || [],
        paidAmount: selectedPurchase.paidAmount || 0,
        paymentMethod: selectedPurchase.paymentMethod || "cash",
  ...
  const totalAmount = watchItems.reduce((acc, item) => {
    return acc + (Number(item.quantity || 0) * Number(item.buyingPrice || 0));
  }, 0);

  const debts = Math.max(0, totalAmount - paidAmount);

  const { mutate, isPending, error, isError } = useMutation({
    mutationFn: (data) => UpdatePurchase(selectedPurchase._id, data),
    onSuccess: () => {
      setIsUpdating(false);
              ...
              <FieldGroup className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Controller
                    ...
                    render={({ field: { onChange, value } }) => (
                      <Autocomplete
                        ...
                        value={suppliersData?.suppliers?.find((s) => s._id === value) || null}
                ...
                <Field>
                  ...
                  <TextField
                    id="purchaseDate"
                    type="date"
                    {...register("purchaseDate", { required: "La date est requise" })}
                  />
                </Field>
              </FieldGroup>
            </FieldSet>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => append({ product: "", quantity: 1, buyingPrice: 0 })}
              ...
              {fields.map((field, index) => (
                <div key={field.id} className="grid grid-cols-1 md:grid-cols-12 gap-3 p-4 bg-slate-50 rounded-lg border border-slate-200 items-end">
                      ...
                      render={({ field: { onChange, value},fieldState: {error} }) => (
                        ...
                        <Autocomplete
                          ...
                          onChange={(_, newValue) => {
                            ...
                            if (newValue) {
                                setValue(`items.${index}.buyingPrice`, newValue.buyingPrice);
                            }
                          }}
                          renderInput={(params) => <TextField {...params} size="small" />}
                        />
            ...
            <FieldSet>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-4 bg-blue-50 rounded-xl border border-blue-100">
                <div className="flex flex-col">
                  <span className="text-sm text-blue-600 font-medium">Total Global</span>
                  <span className="text-2xl font-bold text-blue-900">{totalAmount.toLocaleString()} DH</span>
                </div>
                <div className="flex flex-col">
                ...
                <div className="flex flex-col">
                  <span className="text-sm text-amber-600 font-medium">Dettes</span>
                  <span className="text-2xl font-bold text-amber-700">{debts.toLocaleString()} DH</span>
                </div>
              </div>
